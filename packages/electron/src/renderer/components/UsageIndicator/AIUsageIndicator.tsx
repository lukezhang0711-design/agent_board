    ...
    <div className={`ai-usage-indicator relative ${className || ''}`}>
      <HelpTooltip testId="gutter-ai-usage-button" placement="right">
        <button
          ref={buttonRef}
          onClick={handleClick}
          className={`nav-button relative w-9 h-9 flex items-center justify-center bg-transparent border-none rounded-ui-base cursor-pointer transition-all duration-150 p-0 hover:bg-nim-tertiary active:scale-95 focus-visible:outline-2 focus-visible:outline-[var(--nim-primary)] focus-visible:outline-offset-2 ${isPopoverOpen ? 'bg-nim-tertiary text-nim' : 'text-nim-muted hover:text-nim'}`}
          aria-label="AI 用量"
