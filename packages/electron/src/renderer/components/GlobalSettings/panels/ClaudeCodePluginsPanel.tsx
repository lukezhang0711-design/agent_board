...
function ClaudeCodePluginsPanelInner({ scope = 'user', workspacePath }: ClaudeCodePluginsPanelProps) {
            ...
            <div className="plugin-grid grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3 @container" role="list" aria-label={CATEGORY_LABELS[category] || category}>
              {plugins.map((plugin) => {
                ...
                return (
                  <div
                    key={plugin.name}
                    className={`plugin-card flex flex-col p-4 border rounded-ui-lg cursor-pointer transition-all duration-150 ${installed ? 'installed border-[rgba(39,174,96,0.3)] bg-[rgba(39,174,96,0.05)]' : 'border-[var(--nim-border)] bg-[var(--nim-bg-secondary)]'} hover:border-[var(--nim-primary)] hover:bg-[var(--nim-bg-hover)]`}
                  ...
                  >
                    ...
                    <div className="plugin-card-footer flex items-center justify-between gap-2">
                      ...
                      {installed ? (
                        <span className="plugin-card-badge installed inline-flex items-center px-2 py-1 rounded-ui-base text-[0.6875rem] font-semibold uppercase tracking-tight bg-[rgba(39,174,96,0.15)] text-[#27ae60]">Installed</span>
                      ) : (
                        <button
            ...
            return (
              <div key={itemKey} className="plugin-installed-item flex items-center justify-between p-4 border border-[var(--nim-border)] rounded-ui-lg bg-[var(--nim-bg-secondary)]" role="listitem">
                <div className="plugin-installed-info flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="plugin-installed-details flex-1 min-w-0">
                    <div className="plugin-installed-name flex items-center gap-2 font-medium text-[0.9375rem] text-[var(--nim-text)] mb-1">
                      ...
                      <span
                        className={`plugin-installed-scope inline-flex items-center px-2 py-0.5 rounded-ui-base text-[0.6875rem] font-semibold uppercase tracking-tight ${
                          isProjectScope
                      ...
                      {!plugin.enabled && (
                        <span className="plugin-installed-disabled inline-flex items-center px-2 py-0.5 rounded-ui-base text-[0.6875rem] font-semibold uppercase tracking-tight bg-[rgba(231,76,60,0.12)] text-[#e74c3c]" title="Disabled for this scope">
                          Disabled
                        </span>
                      )}
                    </div>
                ...
                <div className="plugin-installed-actions flex gap-2">
                  <button
                    className="plugin-uninstall-button py-2 px-3 border border-[#e74c3c] rounded-ui-base bg-transparent text-[#e74c3c] text-xs font-medium cursor-pointer transition-all duration-150 hover:bg-[#e74c3c] hover:text-white"
      ...
      <div className="plugin-details-overlay fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-4" onClick={() => setSelectedPlugin(null)}>
        <div className="plugin-details-modal bg-[var(--nim-bg)] rounded-ui-lg p-6 max-w-[500px] w-full max-h-[80vh] overflow-y-auto relative shadow-[0_20px_40px_rgba(0,0,0,0.3)]" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="plugin-details-actions flex items-center gap-3">
            {installed ? (
              <>
                <span className="plugin-details-installed-badge inline-flex items-center py-2 px-3 rounded-ui-base bg-[rgba(39,174,96,0.15)] text-[#27ae60] text-[0.8125rem] font-medium">Installed</span>
                <button
                  className="plugin-uninstall-button py-2 px-3 border border-[#e74c3c] rounded-ui-base bg-transparent text-[#e74c3c] text-xs font-medium cursor-pointer transition-all duration-150 hover:bg-[#e74c3c] hover:text-white"
                  onClick={() => {
              ...
              <button
                className={`plugin-details-install-button flex-1 py-3 px-6 border-none rounded-ui-base bg-[var(--nim-primary)] text-white text-[0.9375rem] font-medium cursor-pointer transition-opacity duration-150 hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed ${status === 'installing' ? 'installing bg-[var(--nim-bg-tertiary)] text-[var(--nim-text-muted)]' : ''}`}
  ...
  return (
    <div className="provider-panel flex flex-col">
      ...
      {/* Status Message */}
      {installMessage && (
        <div className="plugin-status-message py-3 px-4 mb-4 bg-[rgba(52,152,219,0.1)] border border-[rgba(52,152,219,0.3)] rounded-ui-base text-sm text-[var(--nim-text)]" role="status" aria-live="polite">
          {installMessage}
        </div>
      )}
