  ...
  return (
    <div className="diff-preview nim-panel my-3">
      <div className="diff-header nim-panel-header">
        <h3 className="m-0 text-ui-body font-semibold text-[var(--nim-text)]">{title || 'Proposed Changes'}</h3>
        <div className="diff-stats flex gap-3 text-ui-compact font-mono">
      ...
      <div className="diff-content nim-scrollbar p-4 max-h-[400px] overflow-y-auto font-mono text-ui-body leading-relaxed whitespace-pre-wrap break-words">
      ...
      <div className="diff-actions flex gap-3 px-4 py-3 border-t border-[var(--nim-border)] bg-[var(--nim-bg-secondary)]">
        <button className="diff-action-accept flex items-center gap-2 px-4 py-2 rounded-ui-base text-ui-body font-medium cursor-pointer transition-all duration-200 border-none bg-green-500 text-white hover:bg-green-600" onClick={onAccept}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M13 4L6 11L3 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Accept Changes
        </button>
        <button className="diff-action-reject flex items-center gap-2 px-4 py-2 rounded-ui-base text-ui-body font-medium cursor-pointer transition-all duration-200 bg-transparent text-[var(--nim-text-muted)] border border-[var(--nim-border)] hover:bg-[var(--nim-bg-tertiary)] hover:text-[var(--nim-text)]" onClick={onReject}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
