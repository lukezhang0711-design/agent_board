...
export const FilePathBreadcrumb: React.FC<FilePathBreadcrumbProps> = ({
  ...
  return (
    <div className={`unified-header-breadcrumb flex items-center gap-2 text-ui-body min-w-0 overflow-hidden ${className}`.trim()}>
      {breadcrumbSegments.map((segment, index) => {
        ...
        return (
          <React.Fragment key={`${segment.name}-${index}`}>
            ...
            {!isLast && <span className="breadcrumb-separator text-[var(--nim-text-faint)] text-ui-caption">/</span>}
          </React.Fragment>
        );
      })}
    </div>
