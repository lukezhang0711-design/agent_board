...
export const ProjectInsights: React.FC = () => {
  ...
  if (loading) {
    return <div className="project-insights-loading flex items-center justify-center min-h-[400px] text-nim-muted text-ui-subhead">Loading...</div>;
  }

  return (
    <div className="project-insights flex flex-col gap-6">
      <h3 className="m-0 text-ui-title font-semibold text-nim-fg">Usage by Project</h3>
      ...
      {projects.length > 0 ? (
        <div className="project-list grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4">
          {projects.map((project, index) => (
            <div key={index} className="project-card bg-nim-secondary border border-nim rounded-ui-lg p-5">
              <div className="project-name text-ui-subhead font-semibold text-nim-fg mb-4">{project.workspaceId.split('/').pop() || project.workspaceId}</div>
              <div className="project-stats flex flex-col gap-2">
                <div className="project-stat flex justify-between text-ui-body">
        ...
        <div className="no-data flex items-center justify-center min-h-[400px] text-nim-muted text-ui-subhead">No project data available</div>
      )}
    </div>
  );
};
