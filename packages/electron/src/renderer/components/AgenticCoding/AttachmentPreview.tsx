...
export function ProcessingAttachmentPreview({ filename }: ProcessingAttachmentPreviewProps) {
  return (
    <div className="attachment-preview attachment-preview-processing flex items-center gap-2 p-2 border border-[var(--nim-border)] rounded-ui-base bg-[var(--nim-bg-secondary)] min-w-[200px] max-w-[250px] relative transition-colors duration-150 opacity-80">
      <div className="attachment-preview-thumbnail shrink-0 w-10 h-10 flex items-center justify-center rounded-ui-base bg-[var(--nim-bg-tertiary)] overflow-hidden">
...
export function AttachmentPreview({ attachment, onRemove, onConvertToText }: AttachmentPreviewProps) {
  ...
  return (
    <>
      <div className="attachment-preview flex items-center gap-2 p-2 border border-[var(--nim-border)] rounded-ui-base bg-[var(--nim-bg-secondary)] min-w-[200px] max-w-[250px] relative transition-colors duration-150 hover:bg-[var(--nim-bg-hover)]" onContextMenu={handleContextMenu}>
        <div
          className="attachment-preview-thumbnail shrink-0 w-10 h-10 flex items-center justify-center rounded-ui-base bg-[var(--nim-bg-tertiary)] overflow-hidden"
      ...
      <button
        className="attachment-preview-remove shrink-0 w-5 h-5 p-0 border-none bg-transparent cursor-pointer flex items-center justify-center rounded-ui-base transition-colors duration-150 text-[var(--nim-text-faint)] hover:bg-[var(--nim-bg-tertiary)] hover:text-[var(--nim-text)]"
      ...
      {isExpanded && attachment.type === 'image' && (
        ...
        >
          ...
          >
            <button
              className="attachment-preview-modal-close absolute -top-10 right-0 w-8 h-8 p-0 border-none bg-white/10 text-white cursor-pointer flex items-center justify-center rounded-ui-base transition-colors duration-150 hover:bg-white/20"
              onClick={() => setIsExpanded(false)}
              aria-label="Close"
            >
      ...
      {showContextMenu && (
        <div
          ref={contextMenuRef}
          className="attachment-context-menu bg-[var(--nim-bg-secondary)] border border-[var(--nim-border)] rounded-ui-base shadow-[0_4px_12px_rgba(0,0,0,0.15)] py-1 min-w-[140px]"
          style={{
            position: 'fixed',
