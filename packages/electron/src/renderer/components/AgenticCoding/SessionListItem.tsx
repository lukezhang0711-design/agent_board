...
export const SessionListItem = memo<SessionListItemProps>(({
  ...
  return (
    ...
    >
      <div className={`session-list-item-icon shrink-0 mt-1 text-[var(--nim-text-muted)] flex items-center relative ${isActive ? '[&]:text-[var(--nim-primary)] [&_svg]:text-[var(--nim-primary)]' : '[&_svg]:text-[var(--nim-text-muted)]'} ${isWorkstream ? 'workstream-icon' : ''} ${isWorktreeSession ? 'worktree-icon' : ''}`}>
        {sessionType === 'voice' ? (
