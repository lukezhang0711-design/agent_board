...
export const SessionContextMenu: React.FC<SessionContextMenuProps> = ({
  ...
  const menuItemClass = 'session-context-menu-item flex items-center gap-2 w-full px-3 py-2 bg-transparent border-none rounded-ui-base text-[var(--nim-text)] text-ui-body cursor-pointer text-left transition-colors duration-150 hover:bg-[var(--nim-bg-hover)] [&_svg]:shrink-0';
  ...
  return (
    <FloatingPortal>
      <div
        ...
        className="session-context-menu z-[1000] min-w-[140px] p-1 bg-[var(--nim-bg)] border border-[var(--nim-border)] rounded-ui-base shadow-[0_4px_12px_rgba(0,0,0,0.15)]"
      ...
      >
        ...
        >
          ...
          >
            ...
            {phase && (
              <span className="text-ui-micro text-[var(--nim-text-faint)] ml-1">{phase}</span>
            )}
          ...
          {showPhaseSubmenu && (
            <div className={`absolute top-0 min-w-[140px] p-1 bg-[var(--nim-bg)] border border-[var(--nim-border)] rounded-ui-base shadow-[0_4px_12px_rgba(0,0,0,0.15)] z-[1001] ${submenuFlipped ? 'right-full mr-1' : 'left-full ml-1'}`}>
              {SESSION_PHASE_COLUMNS.map((col) => (
                <button
                  key={col.value}
                  className={`session-context-menu-item flex items-center gap-2 w-full px-3 py-2 bg-transparent border-none rounded-ui-base text-ui-body cursor-pointer text-left transition-colors duration-150 hover:bg-[var(--nim-bg-hover)] [&_svg]:shrink-0 ${phase === col.value ? 'text-[var(--nim-primary)]' : 'text-[var(--nim-text)]'}`}
                  onClick={(e) => {
                    e.stopPropagation();
                    onClose();
              ...
              {phase && (
                <>
                  ...
                  <button
                    className="session-context-menu-item flex items-center gap-2 w-full px-3 py-2 bg-transparent border-none rounded-ui-base text-[var(--nim-text-faint)] text-ui-body cursor-pointer text-left transition-colors duration-150 hover:bg-[var(--nim-bg-hover)] [&_svg]:shrink-0"
                    onClick={(e) => {
                      e.stopPropagation();
                      onClose();
        ...
        {onDelete && (
          <button
            className="session-context-menu-item destructive flex items-center gap-2 w-full px-3 py-2 bg-transparent border-none rounded-ui-base text-[var(--nim-error)] text-ui-body cursor-pointer text-left transition-colors duration-150 hover:bg-[var(--nim-error)] hover:text-white [&_svg]:shrink-0"
