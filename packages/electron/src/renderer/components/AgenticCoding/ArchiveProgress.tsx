...
export const ArchiveProgress: React.FC<ArchiveProgressProps> = ({ onWorktreeArchived }) => {
  ...
  return (
    <div className="archive-progress shrink-0 border-t border-[var(--nim-border)] bg-[var(--nim-bg-secondary)]">
      <button
        className="archive-progress-header flex items-center gap-2 px-3 py-3 text-ui-body font-medium text-[var(--nim-text)] bg-transparent border-none w-full cursor-pointer transition-colors duration-150 hover:bg-[var(--nim-bg-tertiary)]"
      ...
      >
        ...
        {activeCount > 0 && (
          <span className="archive-progress-header-count text-ui-body font-medium text-[var(--nim-primary)]">
            {activeCount} active
          </span>
        )}
        <MaterialSymbol
      ...
      {isExpanded && (
        <div className="archive-progress-content flex flex-col">
          ...
          <div className="archive-progress-tasks flex flex-col px-2 pb-2 gap-2">
            {tasks.map((task) => (
              <div
                key={task.worktreeId}
              ...
              >
                ...
                <div className="archive-task-content flex-1 min-w-0 flex flex-col gap-1">
                  <div className="archive-task-name text-ui-body font-medium text-[var(--nim-text)] overflow-hidden text-ellipsis whitespace-nowrap">
                    {task.worktreeName}
                  </div>
                  <div className="archive-task-path text-ui-caption text-[var(--nim-text-faint)] font-[var(--nim-font-mono)] overflow-hidden text-ellipsis whitespace-nowrap">
                    {task.worktreeId}
                  </div>
                  <div
