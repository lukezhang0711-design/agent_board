...
function isBuildInfo(value: unknown): value is BuildInfo {
    ...
    const buildInfo = value as Partial<BuildInfo>;
    return typeof buildInfo.commit === 'string'
        && /^[0-9a-f]{40}$/i.test(buildInfo.commit)
        && typeof buildInfo.shortCommit === 'string'
        && /^[0-9a-f]{7,40}$/i.test(buildInfo.shortCommit)
        && typeof buildInfo.dirty === 'boolean'
        && typeof buildInfo.builtAtUtc === 'string';
}

/** Return null for older/corrupt build outputs so About can render "unknown". */
function readBuildInfo(): BuildInfo | null {
    const appPath = app.getAppPath();
    const candidatePaths = [
        path.join(appPath, 'out', 'build-info.json'),
        // electron-vite's development main process can use out/main as appPath.
        path.join(appPath, '..', 'build-info.json'),
        // Production chunks are nested below out/main/chunks.
        path.join(__dirname, '..', '..', 'build-info.json'),
    ];

    for (const buildInfoPath of candidatePaths) {
        try {
            const parsed = JSON.parse(fs.readFileSync(buildInfoPath, 'utf8'));
            if (isBuildInfo(parsed)) return parsed;
        } catch {
            // Try the next packaging layout; absence must not break About.
        }
    }
...
export function registerSettingsHandlers() {
    ...
    // Build identity is generated by build/build-worker.js and is intentionally
    // separate from the marketing version exposed above.
    safeHandle('get-build-info', () => readBuildInfo());
    ...
    safeHandle('settings:set-default-effort-level', (_event, level: unknown) => {
