...
interface AIServiceInternal {
  ...
  isRuntimeDynamicCatalogProvider?(provider: string): boolean;
...
export class MessageStreamingHandler {
  ...
  /**
   * The production service supplies extension-aware catalog ownership. Keep a
   * built-in fallback for narrow legacy test harnesses that predate that
   * method; real sends always take the service path.
   */
  private isRuntimeDynamicCatalogProvider(provider: string): boolean {
    return this.svc.isRuntimeDynamicCatalogProvider?.(provider)
      ?? (
  ...
  handle: SendMessageHandler = async (
    ...
    if (this.isRuntimeDynamicCatalogProvider(session.provider)) {
      try {
        ...
        // can persist a capability-safe correction without blocking this send.
    ...
    if (!isChildSessionEvent && !isChannelHealthCheck) {
      ...
      if (
        session.metadata?.titleSource !== 'dispatch'
        && (session.messages.length === 0 || (session.messages.length === 1 && session.messages[0].type === 'user_message'))
      ) {
    ...
    if (!provider) {
      ...
      const reinitEffortLevel = this.isRuntimeDynamicCatalogProvider(session.provider)
    ...
    try {
      ...
      if (isClaudeCode) {
        ...
        const turnEffortLevel = this.isRuntimeDynamicCatalogProvider(session.provider)
          ? this.svc.resolveCompatibleDynamicSessionEffortLevel(
              session.provider as AIProviderType,
              session.model || session.providerConfig?.model,
              (session.metadata as any)?.effortLevel,
