type InteractiveResponseMessage = { content: string; createdAt?: Date | string };
...
export function findFreshInteractiveResponse(
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    const createdAt = message.createdAt instanceof Date
      ...
      : typeof message.createdAt === 'string' ? Date.parse(message.createdAt) : Number.NaN;
    ...
    try {
      ...
      const matches = options.matchAnyId === true || options.idFields.some((field) => {
        const value = content[field];
        return typeof value === 'string' && options.acceptedIds.has(value);
      });
      if (matches) return content;
    } catch {
      // Ignore non-JSON transcript rows.
    }
  }
  return null;
}
